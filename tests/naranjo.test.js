...
import {emptyNaranjoAnswers,calculateNaranjo,classifyNaranjo,validateNaranjoRationales,getNaranjoPrerequisites} from '../src/domain/naranjo.js';
...
test('Naranjo requires at least one Suspected drug and one ADR event',()=>{
  assert.equal(getNaranjoPrerequisites({medications:[],events:[]}).ready,false);
  assert.equal(getNaranjoPrerequisites({medications:[{role:'concomitant'}],events:[{id:'e1'}]}).ready,false);
  assert.equal(getNaranjoPrerequisites({medications:[{role:'suspected'}],events:[]}).ready,false);
  const ready=getNaranjoPrerequisites({medications:[{role:'suspected'}],events:[{id:'e1'}]});
  assert.equal(ready.ready,true);
  assert.equal(ready.suspectedCount,1);
